...
Brain::Brain() : rclcpp::Node("brain_node") {
  ...
  declare_parameter<double>("strategy.cooperation.ball_control_cost_threshold",
                            10.0);
  ...
  declare_parameter<double>(
      "obstacle_avoidance.freekick_start_placing_safe_distance", 0.5);
  declare_parameter<double>(
      "obstacle_avoidance.freekick_start_placing_avoid_secs", 1.5);
  ...
  declare_parameter<string>("vision.image_topic",
                            "/camera/camera/color/image_raw");
  declare_parameter<string>("vision.depth_image_topic",
...
void Brain::init() {
  ...
  detectionsSubscription =
      create_subscription<vision_interface::msg::Detections>(
          "/booster_vision/detection", SUB_STATE_QUEUE_SIZE,
          bind(&Brain::detectionsCallback, this, _1));
  subFieldLine = create_subscription<vision_interface::msg::LineSegments>(
      "/booster_vision/line_segments", SUB_STATE_QUEUE_SIZE,
      bind(&Brain::fieldLineCallback, this, _1));
  odometerSubscription = create_subscription<booster_interface::msg::Odometer>(
      "/odometer_state", SUB_STATE_QUEUE_SIZE,
      bind(&Brain::odometerCallback, this, _1));
  lowStateSubscription = create_subscription<booster_interface::msg::LowState>(
      "/low_state", SUB_STATE_QUEUE_SIZE,
      bind(&Brain::lowStateCallback, this, _1));
  headPoseSubscription = create_subscription<geometry_msgs::msg::Pose>(
      "/head_pose", SUB_STATE_QUEUE_SIZE,
      bind(&Brain::headPoseCallback, this, _1));
  recoveryStateSubscription =
      create_subscription<booster_interface::msg::RawBytesMsg>(
          "fall_down_recovery_state", SUB_STATE_QUEUE_SIZE,
  ...
  if (config->rerunLogEnableFile || config->rerunLogEnableTCP) {
    ...
    imageSubscription = create_subscription<sensor_msgs::msg::Image>(
        imageTopic, SUB_STATE_QUEUE_SIZE,
        bind(&Brain::imageCallback, this, _1));
  }
  ...
  depthImageSubscription = create_subscription<sensor_msgs::msg::Image>(
      depthTopic, SUB_STATE_QUEUE_SIZE,
      bind(&Brain::depthImageCallback, this, _1));

  pubSoundPlay = create_publisher<std_msgs::msg::String>("/play_sound", 10);
  pubSpeak = create_publisher<std_msgs::msg::String>("/speak", 10);
  pubLocalizedPose =
      create_publisher<geometry_msgs::msg::PoseStamped>("/localized_pose", 10);
}
...
void Brain::loadConfig() {
  ...
  get_parameter("strategy.ball_confidence_threshold",
                config->ballConfidenceThreshold);
  ...
  get_parameter("obstacle_avoidance.collision_threshold",
  ...
  if (!filesystem::exists(visionConfigPath)) {
    // 报错然后退出
    RCLCPP_ERROR(get_logger(), "vision_config_path %s not exists",
                 visionConfigPath.c_str());
    exit(1);
  }
  // else
  ...
  prtDebug(format("camfx: %f, camfy: %f, camcx: %f, camcy: %f", config->camfx,
                  config->camfy, config->camcx, config->camcy));
...
void Brain::handleSpecialStates() {
  ...
  } else if (msecsSince(data->freekickKickoffStartTime) >
             KICKOFF_DURATION * 1000) {
...
//   data->tmImAlive = (data->penalty[selfIdx] == PENALTY_NONE) &&
...
//                    tmStatus.ballConfidence, tmStatus.ballLocationKnown);
//     }
//   }
...
//       log->setTimeNow();
//       log->log(format("debug/tm_cost_scalar_%d", i + 1),
//                rerun::Scalar(data->tmStatus[i].cost));
//       log->log(format("debug/tm_lead_scalar_%d", i + 1),
//                rerun::Scalar(data->tmStatus[i].isLead));
//     }
//   }
...
//     log_(format("TM %d, ballDetected: %d, ballRange: %.1f", i + 1,
//                 status.ballDetected, status.ballRange));
//     if (status.ballDetected && status.ballRange < minRange) {
//       log_(format("tm ball range(%.1f) < minRange(%.1f)", status.ballRange,
//                   minRange));
//       double dist = norm(status.ballPosToField.x - data->robotPoseToField.x,
...
//     tree->setEntry<string>("player_role", config->playerRole);
//     log_(format("all teammates on field. Back to initial role: %s",
...
void Brain::updateObstacleMemory() {
  ...
  if ((get_parameter("obstacle_avoidance.enable_freekick_avoid").as_bool() &&
...
void Brain::updateBallMemory() {
  ...
  log->setTimeNow();
  log->logBall("field/ball", data->ball.posToField,
               data->ballDetected ? 0x00FF00FF : 0x006600FF, data->ballDetected,
               tree->getEntry<bool>("ball_location_known"));
  log->logBall("field/tmBall", data->tmBall.posToField, 0xFFFF00FF,
...
void Brain::updateRobotMemory() {
  ...
  for (int i = 0; i < robots.size(); i++) {
    ...
    if (msecsSince(r.timePoint) > 1000)
      continue;
...
void Brain::updateKickoffMemory() {
  ...
  auto ballMoved = [=]() {
    ...
    double threshold =
        max(range * BALL_MOVE_THRESHOLD_FACTOR, BALL_MOVE_THRESHOLD_MIN);
    double posChange = norm(data->ball.posToRobot.x - ballPos.x,
                            data->ball.posToRobot.y - ballPos.y);
...
vector<double> Brain::getGoalPostAngles(const double margin) {
  ...
  const double theta_l = atan2(leftY - margin - data->ball.posToField.y,
                               leftX - data->ball.posToField.x);
  const double theta_r = atan2(rightY + margin - data->ball.posToField.y,
                               rightX - data->ball.posToField.x);
...
//     double theta_tm2ball =
//         atan2(status.ballPosToField.y - status.robotPoseToField.y,
//               status.ballPosToField.x - status.robotPoseToField.x);
//     double range_tm2ball =
//         norm(status.ballPosToField.y - status.robotPoseToField.y,
//              status.ballPosToField.x - status.robotPoseToField.x);
...
//     if (range_tm2ball < range_me2ball &&
//         sin(deltaTheta) * range_tm2ball < BUMP_DIST) {
//       cost += 2.0;
...
bool Brain::isAngleGood(double goalPostMargin, string type) {
  double angle = 0;
  if (type == "kick")
    angle = data->robotBallAngleToField; // type=="kick" 机器人到球, field
                                         // 坐标系中的方向
  if (type == "shoot")
    angle = data->robotPoseToField.theta; // type=="shoot" 机器人朝向
...
bool Brain::isPrimaryStriker() {
  ...
  if (firstAliveStrikerIdx >= 0 && firstAliveStrikerIdx < myIdx)
    return false; // 有 id 更小的活的前锋, 让他来当主力
...
bool Brain::isBallOut(double locCompareDist, double lineCompareDist) {
  ...
  for (int i = 0; i < fieldLines.size(); i++) {
    ...
    if ((line.type == LineType::TouchLine || line.type == LineType::GoalLine) &&
        line.confidence > 1.0) {
      Point2D p = {ball.posToField.x, ball.posToField.y};
      // prtWarn(format("Ball: %.2f, %.2f PerpDist: %.2f", ball.posToField.x,
      // ball.posToField.y, pointPerpDistToLine(p, line.posToField)));
      if (pointPerpDistToLine(p, line.posToField) > lineCompareDist)
        return true;
    }
  }
  return false;
...
void Brain::updateBallOut() {
  ...
  threshold +=
      (data->isFreekickKickingOff ? 1.0
                                  : 0.0); // 如果正在踢任意球, 则放宽出界判断
  threshold *=
      (lastBallOut ? 1.0 : 1.5); // 防止震荡. 如果上次判断为出界, 则放宽出界判断
  tree->setEntry<bool>("ball_out", isBallOut(threshold, 10.0) &&
                                       data->ball.range <
                                           range); // 严格通过定位判断是否出界
}
...
double Brain::distToBorder() {
  ...
  auto fd = config->fieldDimensions;
  borders.push_back(
      {fd.length / 2, fd.width / 2, -fd.length / 2, fd.width / 2});
  borders.push_back(
      {fd.length / 2, -fd.width / 2, -fd.length / 2, -fd.width / 2});
  borders.push_back(
      {fd.length / 2, fd.width / 2, fd.length / 2, -fd.width / 2});
...
bool Brain::isBoundingBoxInCenter(BoundingBox boundingBox, double xRatio,
                                  double yRatio) {
...
void Brain::calibrateOdom(double x, double y, double theta) {
  ...
  transCoord(x_or, y_or, theta_or, x, y, theta, data->odomToField.x,
             data->odomToField.y, data->odomToField.theta);

  transCoord(data->robotPoseToOdom.x, data->robotPoseToOdom.y,
             data->robotPoseToOdom.theta, data->odomToField.x,
             data->odomToField.y, data->odomToField.theta,
  ...
  double placeHolder;
  // ball
  transCoord(data->ball.posToRobot.x, data->ball.posToRobot.y, 0,
             data->robotPoseToField.x, data->robotPoseToField.y,
             data->robotPoseToField.theta, data->ball.posToField.x,
             data->ball.posToField.y, placeHolder);
...
void Brain::playSound(string soundName, double blockMsecs, bool allowRepeat) {
  ...
  auto now = get_clock()->now();
  if (msecsSince(_lastTime) < _lastBlockMsecs)
    return;

  if (_lastSound == soundName && (!allowRepeat))
    return;
...
double Brain::msecsSince(rclcpp::Time time) {
  ...
  if (time.get_clock_type() != now.get_clock_type())
    return 1e18;
...
rclcpp::Time Brain::timePointFromHeader(std_msgs::msg::Header header) {
  ...
  // return rclcpp::Time(stamp.sec, stamp.nanosec, RCL_ROS_TIME);  // should
  // crash sometimes
}

void Brain::joystickCallback(
    const booster_interface::msg::RemoteControllerState &joy) {
  auto log_ = [=](string msg) {
    log->setTimeNow();
  ...
  // 通过手柄控制机器人, 不阻塞按键
  if (fabs(joy.lx) > 0.1 || fabs(joy.ly) > 0.1 || fabs(joy.rx) > 0.1 ||
      fabs(joy.ry) > 0.1) {
  ...
  if (joy.lt && !joy.rt) { // LT 组合键
    ...
    if (joy.hat_u || joy.hat_d) {
      ...
      prtDebug(format("vxFactor = %.2f  yawOffset = %.2f", config->vxFactor,
                      config->yawOffset),
               RED_CODE);
    }
...
void Brain::gameControlCallback(
    const game_controller_interface::msg::GameControlData &msg) {
  data->timeLastGamecontrolMsg = get_clock()->now();

  // 处理比赛的一级状态
  auto lastGameState =
      tree->getEntry<string>("gc_game_state"); // 比赛的一级状态
  ...
  bool isKickOffSide =
      (msg.kick_off_team == config->teamId); // 我方是否是开球方
  ...
  vector<string> gameSubStateMap = {
      "STOP", "GET_READY", "SET"}; // STOP: 停下来; -> GET_READY:
                                   // 移动到进攻或防守位置; -> SET: 站住不动
  string gameSubState =
      gameSubStateMap[static_cast<int>(msg.secondary_state_info[1])];
  tree->setEntry<string>("gc_game_sub_state_type", gameSubStateType);
  tree->setEntry<string>("gc_game_sub_state", gameSubState);
  bool isSubStateKickOffSide =
      (static_cast<int>(msg.secondary_state_info[0]) ==
       config->teamId); // 在二级状态下, 我方是否是开球方. 例如,
                        // 当前二级状态为任意球, 我方是否是开任意球的一方
  tree->setEntry<bool>("gc_is_sub_state_kickoff_side", isSubStateKickOffSide);

  // cout << "game state: " << gameState << " game sub state type: " <<
  // gameSubStateType << endl; 找到队的信息
  ...
  if (msg.teams[0].team_number == config->teamId) {
    ...
    prtErr(format("received invalid game controller message team0 %d, team1 "
                  "%d, teamId %d",
                  msg.teams[0].team_number, msg.teams[1].team_number,
                  config->teamId));
    return;
  }
  ...
  int liveCount = 0;
  int oppoLiveCount = 0;
  // 处理判罚状态. penalty[playerId - 1] 代表我方的球员是否处于判罚状态,
  // 处理判罚状态意味着不能移动
  for (int i = 0; i < HL_MAX_NUM_PLAYERS; i++) {
    ...
    if (data->oppoPenalty[i] == PENALTY_NONE)
      oppoLiveCount++;
  }
  data->liveCount = liveCount;
  data->oppoLiveCount = oppoLiveCount;

  // cout << "penalty: " << data->penalty[0] << " " << data->penalty[1] << " "
  // << data->penalty[2] << " " << data->penalty[3] << endl; cout << "oppo
  // penalty: " << data->oppoPenalty[0] << " " << data->oppoPenalty[1] << " " <<
  // data->oppoPenalty[2] << " " << data->oppoPenalty[3] << endl;
  bool lastIsUnderPenalty = tree->getEntry<bool>("gc_is_under_penalty");
  bool isUnderPenalty = (data->penalty[config->playerId - 1] !=
                         PENALTY_NONE); // 当前 robot 是否被判罚中
  tree->setEntry<bool>("gc_is_under_penalty", isUnderPenalty);
  if (isUnderPenalty && !lastIsUnderPenalty)
    tree->setEntry<bool>("odom_calibrated",
                         false); // 被判罚了, 则需要重新进场, 因此需要重新定位
...
void Brain::detectionsCallback(const vision_interface::msg::Detections &msg) {
  ...
  // rclcpp::Time timePoint(detection_time_stamp.sec,
  // detection_time_stamp.nanosec);
  ...
  for (int i = 0; i < gameObjects.size(); i++) {
    ...
    if (obj.label == "Person") {
      ...
      // 为了调试方便, 可以在 config 中设置 treat_person_as_robot, 使得 Person
      // 被当作 Robot 处理
    ...
    if (obj.label == "Opponent")
      robots.push_back(obj);
    if (obj.label == "LCross" || obj.label == "TCross" ||
        obj.label == "XCross" || obj.label == "PenaltyPoint")
...
void Brain::updateLinePosToField(FieldLine &line) {
  ...
  transCoord(line.posToRobot.x0, line.posToRobot.y0, 0,
             data->robotPoseToField.x, data->robotPoseToField.y,
             data->robotPoseToField.theta, line.posToField.x0,
             line.posToField.y0, __);
  transCoord(line.posToRobot.x1, line.posToRobot.y1, 0,
             data->robotPoseToField.x, data->robotPoseToField.y,
             data->robotPoseToField.theta, line.posToField.x1,
...
void Brain::fieldLineCallback(const vision_interface::msg::LineSegments &msg) {
  ...
  { // log processed lines
    ...
    log->log("field/det_lines", rerun::LineStrips2D(logLinesOnField)
    ...
    //         .with_labels(logLabels)
    // );
    log->log("image/det_lines", rerun::LineStrips2D(logLinesOnCam)
                                    .with_colors(logColors)
                                    .with_radii(1.0)
                                    .with_draw_order(20)
                                    .with_labels(logLabels));
...
void Brain::odometerCallback(const booster_interface::msg::Odometer &msg) {
  ...
  //     locator->globalInitPF(
  //         data->robotPoseToOdom); // Initialize with constrained global init
  //   }
...
void Brain::imageCallback(const sensor_msgs::msg::Image &msg) {
  ...
  if (counter % config->rerunLogImgInterval == 0) {
    // 未防止摄像头连接不好时, 自动降低分辨率, 影响 CamTrackBall 的计算,
    // 更新分辨率配置
    config->camPixX = msg.width;
    config->camPixY = msg.height;
    log->log("debug/imageCallback",
             rerun::TextLog(format("img width: %.d, img height: %.d", msg.width,
                                   msg.height)));
    ...
    if (msg.encoding == "nv12" || msg.encoding == "NV12") {
      ...
      if (msg.data.size() < expected) {
        prtErr(format("NV12 buffer too small. got %zu expect >= %zu",
                      msg.data.size(), expected));
        return;
      }
      cv::Mat yuv(msg.height + msg.height / 2, msg.width, CV_8UC1,
                  const_cast<uint8_t *>(msg.data.data()));
      cv::cvtColor(yuv, image, cv::COLOR_YUV2BGR_NV12);
    } else if (msg.encoding == "bgra8") {
      // 创建 OpenCV Mat 对象，处理 BGRA 格式图像
      image = cv::Mat(msg.height, msg.width, CV_8UC4,
                      const_cast<uint8_t *>(msg.data.data()));
      cv::Mat imageBGR;
    ...
    std::vector<int> compression_params = {
        cv::IMWRITE_JPEG_QUALITY, 10}; // 10 表示压缩质量，可以根据需要调整
...
void Brain::recoveryStateCallback(
    const booster_interface::msg::RawBytesMsg &msg) {
  // uint8_t state; // IS_READY = 0, IS_FALLING = 1, HAS_FALLEN = 2,
  // IS_GETTING_UP = 3, uint8_t is_recovery_available; // 1 for available, 0 for
  // not available 使用 RobotRecoveryState
  // 结构，将msg里面的msg转换为RobotRecoveryState
  try {
    ...
    this->data->recoveryState =
        recoveryStateMap[static_cast<int>(recoveryState.state)];
    this->data->isRecoveryAvailable =
        static_cast<bool>(recoveryState.is_recovery_available);
    this->data->currentRobotModeIndex =
        static_cast<int>(recoveryState.current_planner_index);

    // cout << "recoveryState: " << static_cast<int>(recoveryState.state) <<
    // endl; cout << "recovery is available: " <<
    // static_cast<int>(recoveryState.is_recovery_available) << endl; cout <<
    // "current planner idx: " <<
...
int Brain::markCntOnFieldLine(const string markType, const FieldLine line,
                              const double margin) {
  int cnt = 0;
  auto markings = data->getMarkings();
  for (int i = 0; i < markings.size(); i++) {
...
void Brain::identifyFieldLine(FieldLine &line) {
  ...
  for (int i = 0; i < mapLines.size(); i++) {
    ...
    if (mapLine.type == LineType::TouchLine &&
        min(fabs(line.posToField.x0), fabs(line.posToField.x1)) >
            fd.length / 2.0 - fd.penaltyAreaLength - 0.5 &&
        line.posToField.x0 * line.posToField.x1 > 0)
      confidence -= 0.3;

    double length = norm(line.posToField.x0 - line.posToField.x1,
                         line.posToField.y0 - line.posToField.y1);
    if (length < 0.5)
      confidence -= 0.5;
    else if (length < 1.0)
      confidence -= 0.1;
  ...
  if (bestConfidence - secondBestConfidence < 0.5)
    bestConfidence -= 0.5;
...
void Brain::identifyMarking(GameObject &marking) {
  ...
  for (int i = 0; i < config->mapMarkings.size(); i++) {
    ...
    if (mm.type != marking.label)
      continue;

    double dist =
        norm(marking.posToField.x - mm.x, marking.posToField.y - mm.y);
  ...
  auto fd = config->fieldDimensions;
  if (mmIndex >= 0 && mmIndex < config->mapMarkings.size() &&
...
vector<FieldLine> Brain::processFieldLines(vector<FieldLine> &fieldLines) {
  ...
  for (int i = 0; i < original.size(); i++) {
    for (int j = i + 1; j < original.size(); j++) {
      ...
      if (isSameLine(line1, line2, 0.1, 1.0)) {
        ...
        mergedLine.posToRobot =
            mergeLines(original[i].posToRobot, original[j].posToRobot);
        mergedLine.posOnCam =
            mergeLines(original[i].posOnCam, original[j].posOnCam);
        mergedLine.timePoint = original[i].timePoint;

        // replace first line in original with merged line and remove second
        // line
  ...
  // return original;
  // filter out lines that are too short and infer direction while ditch lines
  // whose dir cannot be inferred
  double valve = 0.2;
  for (int i = 0; i < original.size(); i++) {
    auto line = original[i];
    auto lineDir = atan2(line.posToField.y1 - line.posToField.y0,
                         line.posToField.x1 - line.posToField.x0);

    if (fabs(toPInPI(lineDir - M_PI)) < 0.1 || fabs(lineDir) < 0.1)
      line.dir = LineDir::Vertical;
    else if (fabs(toPInPI(lineDir - M_PI / 2)) < 0.1 ||
             fabs(toPInPI(lineDir + M_PI / 2)) < 0.1)
...
vector<GameObject>
Brain::getGameObjects(const vision_interface::msg::Detections &detections) {
  ...
  for (int i = 0; i < detections.detected_objects.size(); i++) {
    ...
    // 深度优先
    // if (obj.position.size() > 0 && !(obj.position[0] == 0 && obj.position[1]
    // == 0)) { // 深度测距成功， 以深度测距为准
    ...
    gObj.pitchToRobot =
        atan2(config->robotHeight, gObj.range); // 注意这是一个近似值

    // 计算对象在 field 坐标系中的位置
    transCoord(
        gObj.posToRobot.x, gObj.posToRobot.y, 0, data->robotPoseToField.x,
        data->robotPoseToField.y, data->robotPoseToField.theta,
        gObj.posToField.x, gObj.posToField.y,
        gObj.posToField.z // 注意, z 没有在其它地方使用, 这里仅为参数占位使用
    );
...
void Brain::detectProcessBalls(const vector<GameObject> &ballObjs) {
  ...
  for (int i = 0; i < ballObjs.size(); i++) {
    ...
    // 如果检测出的球与上次看到的球, 距离和时间都很近, 则对其 confidence
    // 进行适当加分 double c = ballObj.confidence; double oldC =
    // oldBall.confidence; double msecs = msecsSince(oldBall.timePoint); double
    // dist = norm(ballObj.posToField.x - oldBall.posToField.x,
    // ballObj.posToField.y - oldBall.posToField.y); ballObj.confidence += 0.5 *
    // max(oldC, 0.0) * max(1 - msecs/1000, 0.0) * max(1 - dist / 2, 0.0);
    ...
    // 被遮挡的条件要加入. 如果突然消失, 没有遮挡的话, 则只相信一小会儿,
    // 如果有遮挡, 可以相信比较长的时间.
  ...
  if (indexRealBall >= 0) { // 检测到球了
    ...
    // data->ball.confidence = 0; // DO NOT set confidence to 0, confidence
    // decay depends on this.
  }

  // 计算机器人到球的向量, 在 field 坐标系中的方向
  data->robotBallAngleToField =
      atan2(data->ball.posToField.y - data->robotPoseToField.y,
...
void Brain::detectProcessMarkings(const vector<GameObject> &markingObjs) {
  ...
  //    double range = norm(m.posToField.x - data->robotPoseToField.x,
  //    m.posToField.y - data->robotPoseToField.y);
  ...
  log->log("field/identified_markings",
           rerun::LineStrips2D(
               rerun::Collection<rerun::components::LineStrip2D>(circles))
               .with_radii(0.01)
               .with_labels(labels)
...
void Brain::detectProcessRobots(const vector<GameObject> &robotObjs) {
  ...
  //         auto rm = robots[j];
  //         double dist = norm(rm.posToField.x - rbt.posToField.x,
  //         rm.posToField.y - rbt.posToField.y); if (dist < minDist) {
  //             minDist = dist;
  //             minIndex = j;
  //         }
  //     }
...
void Brain::detectProcessVisionBox(
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int end = (i + 1) % 4;
    v.push_back(
        {vbox.posToRobot[end * 2] - vbox.posToRobot[start * 2],
         vbox.posToRobot[end * 2 + 1] - vbox.posToRobot[start * 2 + 1]});
    v.push_back(
        {-vbox.posToRobot[end * 2] + vbox.posToRobot[start * 2],
         -vbox.posToRobot[end * 2 + 1] + vbox.posToRobot[start * 2 + 1]});
  }
  ...
  for (int i = 0; i < 5; i++) {
    ...
    transCoord(xr, yr, 0, data->robotPoseToField.x, data->robotPoseToField.y,
               data->robotPoseToField.theta, xf, yf, __);
...
void Brain::logVisionBox(const rclcpp::Time &timePoint) {
  if (data->visionBox.posToField.size() >= 10) {
    ...
    rerun::LineStrip2D logLines({{v[0], -v[1]},
                                 {v[2], -v[3]},
                                 {v[4], -v[5]},
                                 {v[6], -v[7]},
                                 {v[0], -v[1]}});
    ...
    log->log(
        "field/visionBox",
        rerun::LineStrips2D(logLines).with_colors(0x0000FFCC).with_radii(0.01));
  }
}

void Brain::logDetection(const vector<GameObject> &gameObjects,
                         bool logBoundingBox) {
  if (gameObjects.size() == 0) {
    if (logBoundingBox)
      log->log("image/detection_boxes", rerun::Clear::FLAT);
  ...
  for (int i = 0; i < gameObjects.size(); i++) {
    ...
    labels.push_back(rerun::Text(
        format("%s x:%.2f y:%.2f c:%.1f",
               label == "Opponent" || label == "Person"
                   ? (label + "[" + obj.color + "]").c_str()
                   : label.c_str(),
               obj.posToRobot.x, obj.posToRobot.y, obj.confidence)));
    points.push_back(rerun::Vec2D{
        obj.posToField.x,
        -obj.posToField.y}); // y 取反是因为 rerun Viewer
                             // 的坐标系是左手系。转一下看起来更方便。
  ...
  log->log("field/detection_points",
           rerun::Points2D(points).with_colors(colors).with_radii(radiis)
           // .with_labels(labels)
  );
...
//   // prtDebug(format("logMemRobots called, robotsize = %d", rbts.size()),
//   // RED_CODE);
