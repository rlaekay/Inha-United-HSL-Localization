...
void BrainConfig::calcMapMarkings() {
  ...
  for (auto half : halves) {
    ...
    for (auto side : sides) {
      ...
      mapMarkings.push_back(MapMarking({xSign * fd.length / 2, ySign * fd.width / 2, "LCross", "L" + half + side + "B"}));

      mapMarkings.push_back(MapMarking({xSign * fd.length / 2, ySign * fd.penaltyAreaWidth / 2, "TCross", "T" + half + side + "P"}));

      mapMarkings.push_back(MapMarking({xSign * fd.length / 2, ySign * fd.goalAreaWidth / 2, "TCross", "T" + half + side + "G"}));

      mapMarkings.push_back(MapMarking({xSign * (fd.length / 2 - fd.penaltyAreaLength), ySign * fd.penaltyAreaWidth / 2, "LCross", "L" + half + side + "P"}));

      mapMarkings.push_back(MapMarking({xSign * (fd.length / 2 - fd.goalAreaLength), ySign * fd.goalAreaWidth / 2, "LCross", "L" + half + side + "G"}));
    }
  }
  ...
  for (auto side : sides) {
    ...
    mapMarkings.push_back(MapMarking({0, ySign * fd.width / 2, "TCross", "TM" + side + "B"}));

    mapMarkings.push_back(MapMarking({0, ySign * fd.circleRadius, "XCross", "XM" + side + "C"}));
  }
  ...
  for (auto half : halves) {
    ...
    mapMarkings.push_back(MapMarking({xSign * (fd.length / 2 - fd.penaltyDist), 0, "PenaltyPoint", "P" + half + "MP"}));
  }
}
...
void BrainConfig::handle() {
  ...
  // playerId
  if (playerId < 1 || playerId > HL_MAX_NUM_PLAYERS) { throw invalid_argument("[Error] player_id must be one of [1, .. 11]. Got: " + to_string(playerId)); }
