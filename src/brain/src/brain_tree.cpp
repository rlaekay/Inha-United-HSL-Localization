...
#define REGISTER_BUILDER(Name)                                                                                                                                 \
  factory.registerBuilder<Name>(#Name, [this](const string &name, const NodeConfig &config) { return make_unique<Name>(name, config, brain); });
...
NodeStatus SetVelocity::tick() {
  ...
  // [NEW] Disable LocalPlanner to prevent ghosting
  PlannerTarget target;
  brain->setMotionCommand(ControlMode::IDLE, target);
...
NodeStatus CamTrackBall::tick() {
  ...
  auto logTrackingBox = [=](int color, string label) {
    ...
    brain->log->log("image/track_ball", rerun::Boxes2D::from_mins_and_sizes(mins, sizes).with_labels({label}).with_colors(color));
  };

  bool iSeeBall = brain->data->ballDetected;
  bool iKnowBallPos = brain->tree->getEntry<bool>("ball_location_known");
  bool tmBallPosReliable = brain->tree->getEntry<bool>("tm_ball_pos_reliable");
  if (!(iKnowBallPos || tmBallPosReliable)) return NodeStatus::SUCCESS;
...
NodeStatus CamScanField::tick() {
  ...
  double yaw = cycleTime < (msecCycle / 2.0) ? (leftYaw - rightYaw) * (2.0 * cycleTime / msecCycle) + rightYaw
...
NodeStatus Chase::tick() {
  ...
  double theta_br = atan2(brain->data->robotPoseToField.y - brain->data->ball.posToField.y, brain->data->robotPoseToField.x - brain->data->ball.posToField.x);
  ...
  // Calculate Target (Geometry and Logic only)
  ...
  // [NEW] Use LocalPlanner to drive
  PlannerTarget target;
  target.pose.x = target_f.x;
  target.pose.y = target_f.y;
  target.pose.theta = atan2(target_f.y - brain->data->robotPoseToField.y, target_f.x - brain->data->robotPoseToField.x); // Face target

  // For Chase, we typically want aggressive behavior, but now configurable
  string navModeStr;
  getInput("nav_mode", navModeStr);
  ControlMode mode = ControlMode::STEAL; // Default

  if (navModeStr == "DRIBBLE") {
    mode = ControlMode::DRIBBLE;
  } else if (navModeStr == "CONTEST_BALL") {
    mode = ControlMode::CONTEST_BALL;
  } else if (navModeStr == "STEAL") {
    mode = ControlMode::STEAL;
  } else if (navModeStr == "NAV_TO_POINT" || navModeStr == "POINT") {
    mode = ControlMode::NAV_TO_POINT;
  } else if (navModeStr == "NAV_TO_POSE" || navModeStr == "POSE") {
    mode = ControlMode::NAV_TO_POSE;
  }

  brain->setMotionCommand(mode, target);
...
NodeStatus GoToFreekickPosition::onRunning() {
  ...
  if (side == "attack") {
    ...
    if (brain->config->numOfPlayers == 3 && brain->data->liveCount >= 2) {
      if (!brain->isPrimaryStriker()) { targetPose.y = targetPose.y > 0 ? targetPose.y - 1.0 : targetPose.y + 1.0; }
    }
  }
  ...
  // brain->log->log("debug/freekick_position", rerun::TextLog(format("stage move: targetPose: (%.2f, %.2f, %.2f)", targetPose.x, targetPose.y,
  // targetPose.theta)));
  brain->client->moveToPoseOnField3(targetPose.x, targetPose.y, targetPose.theta, longRangeThreshold, turnThreshold, vxLimit, vyLimit, vthetaLimit, 0.2, 0.2,
                                    0.1, avoidObstacle);
...
NodeStatus GoToGoalBlockingPosition::tick() {
  ...
  Pose2D targetPose;
  targetPose.x = curRole == "striker" ? (std::max(-fd.length / 2.0 + distToGoalline, ballPos.x - 1.5)) : (-fd.length / 2.0 + distToGoalline);
  ...
  double dist = norm(targetPose.x - robotPose.x, targetPose.y - robotPose.y);
  if ( // 认为到达了目标位置
      dist < distTolerance && fabs(brain->data->ball.yawToRobot) < thetaTolerance) {
    brain->client->setVelocity(0, 0, 0);
    return NodeStatus::SUCCESS;
  }
...
NodeStatus Adjust::tick() {
  ...
  log("enter");
  if (!brain->tree->getEntry<bool>("ball_location_known")) { return NodeStatus::SUCCESS; }
...
NodeStatus CalcKickDir::tick() {
  ...
  brain->log->setTimeNow();
  brain->log->log("field/kick_dir", rerun::Arrows2D::from_vectors({{10 * cos(brain->data->kickDir), -10 * sin(brain->data->kickDir)}})
...
NodeStatus StrikerDecide::tick() {
  ...
  log(format("ballRange: %.2f, ballYaw: %.2f, ballX:%.2f, ballY: %.2f kickDir: %.2f, dir_rb_f: %.2f, angleGoodForKick: %d", ballRange, ballYaw, ballX, ballY,
             kickDir, dir_rb_f, angleGoodForKick));
  ...
  if (!(iKnowBallPos || tmBallPosReliable)) {
    ...
    newDecision = "chase";
    color = 0x0000FFFF;
  } else if (((angleGoodForKick && !brain->data->isFreekickKickingOff) || reachedKickDir) && brain->data->ballDetected &&
             fabs(brain->data->ball.yawToRobot) < M_PI / 2. && !avoidKick && ball.range < 1.5) {
  ...
  brain->log->logToScreen("tree/Decide",
                          format("Decision: %s ballrange: %.2f ballyaw: %.2f kickDir: %.2f rbDir: %.2f angleGoodForKick: %d lead: %d", newDecision.c_str(),
                                 ballRange, ballYaw, kickDir, dir_rb_f, angleGoodForKick, brain->data->tmImLead),
                          color);
  return NodeStatus::SUCCESS;
}
...
NodeStatus GoalieDecide::tick() {
  ...
  setOutput("decision_out", newDecision);
  brain->log->logToScreen("tree/Decide",
                          format("Decision: %s ballrange: %.2f ballyaw: %.2f kickDir: %.2f rbDir: %.2f angleIsGood: %d", newDecision.c_str(), ballRange,
                                 ballYaw, kickDir, dir_rb_f, angleIsGood),
                          color);
  return NodeStatus::SUCCESS;
}
...
NodeStatus Kick::onRunning() {
  ...
  if (enableAbort &&
      ((brain->data->ballDetected && ballRange - _minRange > MOVE_RANGE_THRESHOLD) || brain->msecsSince(brain->data->ball.timePoint) > BALL_LOST_THRESHOLD)) {
  ...
  double kickAoSafeDist;
  brain->get_parameter("obstacle_avoidance.kick_ao_safe_dist", kickAoSafeDist);
  if (avoidPushing && brain->data->robotPoseToField.x < brain->config->fieldDimensions.length / 2 - brain->config->fieldDimensions.goalAreaLength &&
      brain->distToObstacle(brain->data->ball.yawToRobot) < kickAoSafeDist) {
    brain->client->setVelocity(-0.1, 0, 0);
    return NodeStatus::SUCCESS;
  }
...
NodeStatus StandStill::onStart() {
  ...
  // [NEW] Stop the LocalPlanner to prevent it from overwriting or ghosting
  PlannerTarget target; // Empty target
  brain->setMotionCommand(ControlMode::IDLE, target);
...
NodeStatus MoveToPoseOnField::tick() {
  ...
  brain->client->moveToPoseOnField2(tx, ty, ttheta, longRangeThreshold, turnThreshold, vxLimit, vyLimit, vthetaLimit, xTolerance, yTolerance, thetaTolerance,
                                    avoidObstacle);
  return NodeStatus::SUCCESS;
}
...
NodeStatus GoToReadyPosition::tick() {
  ...
  brain->client->moveToPoseOnField2(tx, ty, ttheta, longRangeThreshold, turnThreshold, vxLimit, vyLimit, vthetaLimit, distTolerance / 1.5, distTolerance / 1.5,
                                    thetaTolerance, avoidObstacle);
  return NodeStatus::SUCCESS;
}
...
NodeStatus CheckAndStandUp::tick() {
  ...
  brain->log->log("recovery", rerun::TextLog(format("Recovery retry count: %d, recoveryPerformed: %d recoveryState: %d currentRobotModeIndex: %d",
                                                    brain->data->recoveryPerformedRetryCount, brain->data->recoveryPerformed, brain->data->recoveryState,
                                                    brain->data->currentRobotModeIndex)));

  if (!brain->data->recoveryPerformed && brain->data->recoveryState == RobotRecoveryState::HAS_FALLEN &&
      // brain->data->isRecoveryAvailable &&
      brain->data->currentRobotModeIndex == 1 && brain->data->recoveryPerformedRetryCount < brain->get_parameter("recovery.retry_max_count").get_value<int>()) {
  ...
  if (brain->data->recoveryState == RobotRecoveryState::IS_READY && brain->data->currentRobotModeIndex == 8) {
    brain->data->recoveryPerformedRetryCount = 0;
    brain->data->recoveryPerformed = false;
...
NodeStatus PrintMsg::tick() {
  ...
  if (!msg) { throw RuntimeError("missing required input [msg]: ", msg.error()); }
...
NodeStatus Go::tick() {
  ...
  getInput("y", y);
  getInput("theta", theta);
  getInput("mode", modeStr);

  ControlMode mode = ControlMode::NAV_TO_POSE; // Default
  if (modeStr == "navigation")
    mode = ControlMode::NAV_TO_POSE;
  else if (modeStr == "dribble")
    mode = ControlMode::DRIBBLE;
  else if (modeStr == "contest")
    mode = ControlMode::CONTEST_BALL;
  else if (modeStr == "steal")
    mode = ControlMode::STEAL;

  PlannerTarget target;
  target.pose = {x, y, theta};
  // target.point = {x, y}; // Also populate point for NAV_TO_POINT if needed
